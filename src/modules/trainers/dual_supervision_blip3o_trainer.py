"""
Enhanced BLIP3-o Trainer with Dual Supervision Support
Place this file as: src/modules/trainers/dual_supervision_blip3o_trainer.py

Handles dual supervision training with:
1. Patch-level reconstruction loss
2. Global alignment loss (in CLIP 768-dim space)
3. Flow matching loss
4. Enhanced metrics tracking for recall performance
"""
...
class DualSupervisionBLIP3oTrainer(Trainer):
    """
    Enhanced trainer for dual supervision BLIP3-o training.
    
    Handles:
    - Dual model outputs (patch + global)
    - Dual supervision loss computation
    - CLIP projection for target global features
    ...
    def compute_target_global_features(self, clip_embeddings: torch.Tensor) -> torch.Tensor:
        """
        Compute target global features in CLIP's 768-dim aligned space.
    ...
    def compute_loss(
        ...
        """
        Compute dual supervision loss with enhanced metrics.
        
        This implements the dual supervision training procedure:
        1. Extract EVA-CLIP conditioning and CLIP targets
        2. Forward through model to get both patch and global outputs
        3. Compute target global features using CLIP projection
        4. Compute dual supervision loss (patch + global + flow matching)
        """
        # Extract inputs
    ...
    def evaluate(
        ...
        """Enhanced evaluation with dual supervision metrics."""
    ...
    def save_model(self, output_dir: Optional[str] = None, _internal_call: bool = False):
        ...
        logger.info(f"Dual supervision BLIP3-o model saved to {output_dir}")
    ...
    def _save_dual_supervision_configs(self, output_dir: Path):
        ...
        dual_summary = {
            'architecture': 'dual_supervision_blip3o_dit',
        ...
        with open(output_dir / 'dual_supervision_summary.json', 'w') as f:
        ...
        for key, values in self.loss_components.items():
            if values:
                recent_values = values[-100:]  # Last 100 steps
                loss_analysis[key] = {
                    'mean': np.mean(recent_values),
                    'std': np.std(recent_values),
                    'trend': np.mean(recent_values[-25:]) - np.mean(recent_values[-50:-25]) if len(recent_values) >= 50 else 0,
                    'final_value': recent_values[-1],
                }
        
        with open(output_dir / 'dual_supervision_loss_analysis.json', 'w') as f:
...
def create_blip3o_training_args(
    ...
    **kwargs
    ...
    """Create TrainingArguments optimized for dual supervision training."""
