...
if __name__ == "__main__":
    ...
    """
Evaluation metrics for BLIP3-o DiT model evaluation.
FIXED: Added missing compute_pairwise_cosine_similarity function and improved normalization checks.
UPDATED: Added support for CLIP visual projection validation and debugging.
"""
...
def compute_pairwise_cosine_similarity(
    ...
    """
    FIXED: Added missing function for pairwise cosine similarity computation.
    
    Compute pairwise cosine similarity between two sets of embeddings.
    This was the missing function causing the recall evaluation to fail!
...
def validate_embedding_normalization(
    embeddings: torch.Tensor,
    name: str = "embeddings",
    tolerance: float = 1e-5
) -> Dict[str, any]:
    """
    ADDED: Validate that embeddings are properly normalized.
    
    This helps debug normalization issues that cause low recall scores.
    
    Args:
        embeddings: Embeddings to validate [N, D]
    ...
    with torch.no_grad():
        ...
        # Check if normalized (norms should be close to 1.0)
        expected_norm = 1.0
        norm_diff = torch.abs(norms - expected_norm)
        is_normalized = torch.all(norm_diff < tolerance)
        ...
        validation_results = {
            ...
            'max_norm_deviation': norm_diff.max().item(),
            'tolerance': tolerance,
            'name': name,
        ...
        # Log results
        if is_normalized:
            logger.debug(f"✅ {name} are properly normalized (max deviation: {norm_diff.max():.2e})")
        else:
            logger.warning(f"⚠️  {name} are NOT properly normalized (max deviation: {norm_diff.max():.2e})")
            logger.warning(f"   Norm stats - mean: {norms.mean():.6f}, std: {norms.std():.6f}")
...
def compute_recall_metrics(
    ...
    """
    Compute Recall@K metrics for retrieval evaluation.
    FIXED: Now includes proper normalization validation.
    ...
    Returns:
        Dictionary containing Recall@K scores
    """
    # FIXED: Validate embeddings are normalized
    query_validation = validate_embedding_normalization(query_embeddings, "query_embeddings")
    gallery_validation = validate_embedding_normalization(gallery_embeddings, "gallery_embeddings")
    
    if not query_validation['is_normalized']:
        logger.warning("Query embeddings are not normalized! This may cause poor recall performance.")
    if not gallery_validation['is_normalized']:
        logger.warning("Gallery embeddings are not normalized! This may cause poor recall performance.")
...
def compute_alignment_metrics(
    ...
    """
    Compute alignment metrics between text and vision embeddings.
    FIXED: Now includes normalization validation.
    ...
    """
    # FIXED: Validate embeddings are normalized
    text_validation = validate_embedding_normalization(text_embeddings, "text_embeddings")
    vision_validation = validate_embedding_normalization(vision_embeddings, "vision_embeddings")
    
    if not text_validation['is_normalized']:
        logger.warning("Text embeddings are not normalized! This may cause poor alignment scores.")
    if not vision_validation['is_normalized']:
        logger.warning("Vision embeddings are not normalized! This may cause poor alignment scores.")
...
def compute_retrieval_accuracy(
    ...
    """
    # FIXED: Validate embeddings are normalized
    validate_embedding_normalization(query_embeddings, "query_embeddings")
    validate_embedding_normalization(gallery_embeddings, "gallery_embeddings")
...
def debug_similarity_computation(
    ...
    """
    ADDED: Debug function to analyze similarity computation in detail.
    
    This helps identify issues with embedding normalization and similarity calculation.
    
    Args:
        embeddings_a: First set of embeddings [N, D]
        embeddings_b: Second set of embeddings [M, D]
        sample_indices: Indices to analyze in detail (if None, use first 5)
    ...
    if sample_indices is None:
        sample_indices = list(range(min(5, len(embeddings_a), len(embeddings_b))))
    
    debug_info = {}
    
    with torch.no_grad():
        # Validate normalization
        norm_a = validate_embedding_normalization(embeddings_a, name_a)
        norm_b = validate_embedding_normalization(embeddings_b, name_b)
        
        debug_info['normalization'] = {
            f'{name_a}_normalized': norm_a['is_normalized'],
            f'{name_b}_normalized': norm_b['is_normalized'],
            f'{name_a}_mean_norm': norm_a['mean_norm'],
            f'{name_b}_mean_norm': norm_b['mean_norm'],
        }
        
        # Compute similarity matrix
        similarity_matrix = compute_pairwise_cosine_similarity(embeddings_a, embeddings_b)
        
        debug_info['similarity_stats'] = {
        ...
        debug_info['sample_analysis'] = {}
        for i, idx in enumerate(sample_indices):
            if idx < len(embeddings_a):
                similarities = similarity_matrix[idx]
                top_indices = torch.argsort(similarities, descending=True)[:3]
                
                debug_info['sample_analysis'][f'sample_{idx}'] = {
                    'top_3_similarities': similarities[top_indices].tolist(),
                    'top_3_indices': top_indices.tolist(),
                    'embedding_norm': torch.norm(embeddings_a[idx]).item(),
                }
...
def print_debug_info(debug_info: Dict[str, any], title: str = "Debug Information"):
    """
    ADDED: Pretty print debug information from similarity computation analysis.
    
    Args:
        debug_info: Debug information from debug_similarity_computation
        title: Title for the debug report
    """
    print(f"\n🐛 {title}")
    print("=" * (len(title) + 4))
    
    # Normalization info
    if 'normalization' in debug_info:
        norm_info = debug_info['normalization']
    ...
    # Similarity stats
    if 'similarity_stats' in debug_info:
        sim_info = debug_info['similarity_stats']
        print("\n   Similarity Matrix Stats:")
        print(f"      Shape: {sim_info['shape']}")
        print(f"      Mean: {sim_info['mean']:.4f}")
        print(f"      Std: {sim_info['std']:.4f}")
        print(f"      Range: [{sim_info['min']:.4f}, {sim_info['max']:.4f}]")
    
    # Sample analysis
    if 'sample_analysis' in debug_info:
        print("\n   Sample Analysis:")
        for sample_name, sample_info in debug_info['sample_analysis'].items():
            print(f"      {sample_name}:")
            print(f"         Top 3 similarities: {[f'{x:.4f}' for x in sample_info['top_3_similarities']]}")
            print(f"         Top 3 indices: {sample_info['top_3_indices']}")
            print(f"         Embedding norm: {sample_info['embedding_norm']:.6f}")
...
if __name__ == "__main__":
    # Test the metrics functions
    print("🧪 Testing FIXED evaluation metrics with proper normalization...")
    ...
    similarity_unnorm = compute_cosine_similarity(embeddings_a, embeddings_b)
    print(f"Similarity (auto-normalized): {similarity_unnorm.mean():.4f} ± {similarity_unnorm.std():.4f}")
    
    # Test validation
    validation_a = validate_embedding_normalization(embeddings_a, "test_embeddings_a")
    print(f"Validation A: normalized={validation_a['is_normalized']}, mean_norm={validation_a['mean_norm']:.6f}")
    
    # Test pairwise similarity (the missing function!)
    embeddings_c = torch.randn(50, 768)
    embeddings_d = torch.randn(100, 768)
    
    pairwise_sim = compute_pairwise_cosine_similarity(embeddings_c, embeddings_d)
    print(f"✅ Pairwise similarity matrix shape: {pairwise_sim.shape}")
    print(f"   Similarity range: [{pairwise_sim.min():.4f}, {pairwise_sim.max():.4f}]")
    
    # Test debug function
    debug_info = debug_similarity_computation(embeddings_c, embeddings_d, name_a="Query", name_b="Gallery")
    print_debug_info(debug_info, "Similarity Debug Test")
    ...
    print("✅ All FIXED metric tests passed!")
    print("🎯 Key fixes applied:")
    print("   • Added missing compute_pairwise_cosine_similarity function")
    print("   • Added embedding normalization validation")
    print("   • Added debug functions for similarity analysis")
    print("   • Improved error handling and device consistency")